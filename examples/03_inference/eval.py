...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Enable Weights & Biases logging during evaluation.",
)
...
def main():
    ...
    try:
        ...
        if args_cli.episode_length_s is not None and args_cli.episode_length_s <= 0:
            raise ValueError("--episode_length_s must be greater than zero.")

        task_spec = gym.spec(args_cli.task)
        agent_policy_path = args_cli.checkpoint or task_spec.kwargs.get("best_model_path")
        ...
        if args_cli.dataset_name is not None:
            env_cfg = configure_datarecorder(
                ...
                args_cli.dataset_type,
            )

        # key = agent name, value = path to config file
        experiment_cfg_file = task_spec.kwargs.get("skrl_cfgs")[args_cli.agent.upper()]
